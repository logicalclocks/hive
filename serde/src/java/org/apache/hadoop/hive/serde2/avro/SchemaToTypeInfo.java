...
class SchemaToTypeInfo {
  ...
  public static TypeInfo generateTypeInfo(Schema schema,
    ...
    if (type == BYTES && AvroSerDe.DECIMAL_TYPE_NAME
      ...
      int scale = 0;
      try {
        precision = ((Number) schema.getObjectProp(AvroSerDe.AVRO_PROP_PRECISION)).intValue();
        scale = ((Number) schema.getObjectProp(AvroSerDe.AVRO_PROP_SCALE)).intValue();
      } catch (Exception ex) {
    ...
    if (type == STRING &&
      ...
      int maxLength = 0;
      try {
        maxLength = ((Number) schema.getObjectProp(AvroSerDe.AVRO_PROP_MAX_LENGTH)).intValue();
      } catch (Exception ex) {

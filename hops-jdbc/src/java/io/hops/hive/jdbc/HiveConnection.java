...
public class HiveConnection implements java.sql.Connection {
  ...
  private CloseableHttpClient getHttpClient(Boolean useSsl) throws SQLException {
    ...
    if (isKerberosAuthMode()) {
      ...
      requestInterceptor = new HttpKerberosRequestInterceptor(
          ...
          loggedInSubject, cookieStore, cookieName, useSsl, additionalHttpHeaders, customCookies);
    } else {
  ...
  private void openSession() throws SQLException, IOException {
    ...
    // TODO: HopsFS 4.x - keyStore/trustStore fields not yet in TOpenSessionReq thrift schema;
    // pass via configuration map when server-side handling is added.
    ...
    try {
      ...
      if (!supportedProtocols.contains(openResp.getServerProtocolVersion())) {
        throw new TException("Unsupported hopshive protocol");
      }
